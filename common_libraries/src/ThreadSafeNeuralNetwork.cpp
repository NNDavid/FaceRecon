...
ThreadSafeNeuralNetwork::ThreadSafeNeuralNetwork() = default;

ThreadSafeNeuralNetwork::ThreadSafeNeuralNetwork(const std::string& model_name, const std::string& config_name)
{
...
void ThreadSafeNeuralNetwork::evaluate(const cv::Mat& input, /*output*/ std::vector<Output>& faceResult)
{
	faceResult.clear();
